...
def _md_to_flowables(md_text: str, styles: dict, table_charts: dict = None) -> list:
    ...
    조판 규칙(페이지 경계 쪼개짐·과도 공백 방지):
      - 캡션+차트는 KeepTogether로 묶어 캡션 고아를 막되, 차트가 있는 캡션의 표는 별도 블록으로
        둔다 — 캡션+차트+표를 통짜로 묶으면 페이지 중간에 못 들어가 하단에 큰 공백이 남는다.
      - 차트가 없는 캡션 + 작은 표는 한 덩어리. 작은 표(≤ SMALL_TABLE_MAX_ROWS)는 단독으로도
        통짜 유지하고, 긴 표는 분할을 허용한다(repeatRows=1로 헤더 반복).
      - 섹션 제목은 '다음 블록'에 합류시켜 고아를 방지한다. ParagraphStyle.keepWithNext는
        다음이 KeepTogether일 때 무시되므로 코드가 직접 끼워 넣는다."""
